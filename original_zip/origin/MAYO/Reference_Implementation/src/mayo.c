// modified by Alexandre Stordeur the 12/02/26 
...
// decode a string x to output a vector
...
// encode a verctor x as a string
...
// Algorithm 5 MAYO.ExpandSK(csk) expend the secret key
...
// Algorithm 10 MAYO.API.sign open(pk, M, sig): Takes as input a signed message sm ∈ B∗ and a public key
// pk ∈ Bcpk bytes. It first calls MAYO.ExpandPK to expand the public key, and then it calls MAYO.Verify
// with the expanded public key to check if the signature is valid. It outputs the result of MAYO.Verify
// and if the signature is valid it also outputs the original message.
...
// Algorithm 4 MAYO.CompactKeyGen() output compact keys
