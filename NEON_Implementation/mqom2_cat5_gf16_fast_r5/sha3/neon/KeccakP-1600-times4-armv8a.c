...
{
    KeccakP1600times4_states *s = states;
    KeccakP1600times4_permute24((uint64_t *)s->states, KeccakP1600times4_RC);
}
...
size_t KeccakF1600times4_FastLoop_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    ...
    while (dataByteLen >= (laneOffsetParallel*3 + laneCount)*SnP_laneLengthInBytes) {
        KeccakP1600times4_AddLanesAll((KeccakP1600times4_states *)states, data, laneCount, laneOffsetParallel);
...
size_t KeccakP1600times4_12rounds_FastLoop_Absorb(void *states, unsigned int laneCount, unsigned int laneOffsetParallel, unsigned int laneOffsetSerial, const unsigned char *data, size_t dataByteLen)
{
    ...
    while (dataByteLen >= (laneOffsetParallel*3 + laneCount)*SnP_laneLengthInBytes) {
        KeccakP1600times4_AddLanesAll((KeccakP1600times4_states *)states, data, laneCount, laneOffsetParallel);
        KeccakP1600times4_PermuteAll_12rounds((KeccakP1600times4_states *)states);
        data += laneOffsetSerial*SnP_laneLengthInBytes;
        dataByteLen -= laneOffsetSerial*SnP_laneLengthInBytes;
    }
    return data - dataStart;
