...
int main(int argc, char *argv[]) {
    ...
    printf("   - Key Gen: %.2f cycles", btimer_get_cycles(&timers_algos[B_KEY_GENERATION]));
    printf("  - Sign:    %.2f cycles", btimer_get_cycles(&timers_algos[B_SIGN_ALGO]));
    printf("  - Verify:  %.2f cycles\n", btimer_get_cycles(&timers_algos[B_VERIFY_ALGO]));
    printf("\n");
    #endif

    printf("Communication cost:");
    printf(" - PK size: %ld B", CRYPTO_PUBLICKEYBYTES);
    printf(" - SK size: %ld B", CRYPTO_SECRETKEYBYTES);
    printf(" - Signature size (MAX): %ld B", CRYPTO_BYTES);
