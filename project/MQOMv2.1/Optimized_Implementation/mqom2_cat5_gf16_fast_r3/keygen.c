...
int KeyGen(const uint8_t seed_key[2 * MQOM2_PARAM_SEED_SIZE], uint8_t sk[MQOM2_SK_SIZE], uint8_t pk[MQOM2_PK_SIZE])
{
    ...
    // -------------------------------------------------------------------------
...
#if defined(CYCLE_MAIN_LOOP)
        static uint64_t sum_ab_aa = 0;
        static uint64_t sum_ac_ab = 0;
        static uint64_t sum_ad_ac = 0;
        ...
        if (i == (MQOM2_PARAM_MQ_M / MQOM2_PARAM_MU) - 1) {

            printf("MQOM KeyGen loop cycles, over %llu iters, %llu, %llu, %llu\n",
