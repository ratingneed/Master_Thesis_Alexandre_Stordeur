...
#include <valgrind/memcheck.h>
    ...
    #include "../include/timing_and_stat.h"
#endif
...
int mayo_sign_signature(const mayo_params_t *p, unsigned char *sig,
...
#if defined(CYCLE_MAIN_LOOP)
        uint64_t af_time = x86_64_rtdsc();
#endif

#if defined(CYCLE_MAIN_LOOP)
    static uint64_t sum_ab_aa = 0, sum_ac_ab = 0, sum_ad_ac = 0, sum_ae_ad = 0, sum_af_ae = 0;
    ...
    sum_af_ae += (af_time - ae_time);

    iters++;

    if (ctr == 255) {
        printf("Sign loop 1 cycles, over %llu iters, %llu, %llu, %llu, %llu, %llu\n",
           ...
           (unsigned long long)(sum_af_ae / iters));
...
err:
    ...
    for (uint64_t i = 1; i < sizeof(t1)/sizeof(t1[0]); i++) {
        printf(", %d", (int)(t1[i] - t1[i-1]));
    }
...
err:
    ...
    uint64_t t[] = {a_time, b_time, c_time, d_time, e_time, f_time, g_time, h_time, i_time, j_time};
...
int mayo_verify(const mayo_params_t *p, const unsigned char *m,
    ...
    // row 29-31
    // -------------------------------------------------------------------------
    int memcmp_result = memcmp(y, t, param_m);
    // -------------------------------------------------------------------------
    ...
    for (uint64_t i = 1; i < sizeof(t_cycles)/sizeof(t_cycles[0]); i++) {
        printf(", %d", (int)(t_cycles[i] - t_cycles[i-1]));
    }
